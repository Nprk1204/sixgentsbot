...
connected_users = {}  # user_id -> {sid, rooms}
...
def handle_join_user_room(data):
    ...
    try:
        ...
        # Send initial status
        status = get_player_queue_status(user_id)
...
def handle_join_queue_room(data):
    """Join user to queue monitoring room"""
    try:
        channel_id = data.get('channel_id')
        if not channel_id:
            return

        join_room(f'queue_{channel_id}')

        # Send current queue status for this channel
        queue_status = get_channel_queue_status(channel_id)
        emit('queue_update', queue_status)

        print(f"Client joined queue room: {channel_id}")

    except Exception as e:
        print(f"Error in join_queue_room: {e}")
...
def get_player_queue_status(player_id):
    ...
    try:
        ...
        active_match = matches_collection.find_one({
            "$or": [
                {"team1.id": player_id},
                {"team2.id": player_id},
        ...
        if active_match:
            match_status = active_match.get("status", "in_progress")
            return {
                "status": "in_match",
                "match_id": active_match.get("match_id"),
                "match_status": match_status,
                "is_global": active_match.get("is_global", False),
                "phase": get_match_phase_display(match_status),
                "description": get_match_description(match_status),
                "created_at": active_match.get("created_at").isoformat() if active_match.get("created_at") else None,
                "timestamp": datetime.datetime.utcnow().isoformat()
            }

        # Check if player is in queue
        queue_entry = queue_collection.find_one({"id": player_id})

        if queue_entry:
            channel_id = queue_entry.get("channel_id")
            is_global = queue_entry.get("is_global", False)
            joined_at = queue_entry.get("joined_at")

            # Get queue position and total
            queue_players = list(queue_collection.find(
                {"channel_id": channel_id}
            ).sort("joined_at", 1))

            position = next((i + 1 for i, p in enumerate(queue_players) if p.get("id") == player_id), None)
            total = len(queue_players)
            ...
            return {
                "status": "in_queue",
                "queue_type": "global" if is_global else "ranked",
                "channel_id": channel_id,
                "position": position,
                "total_in_queue": total,
                "time_in_queue": time_in_queue,
                "joined_at": joined_at.isoformat() if joined_at else None,
                "description": f"In queue ({position}/{total})",
                "estimated_wait": estimate_queue_wait_time(total),
                "players_needed": max(0, 6 - total),
                "timestamp": datetime.datetime.utcnow().isoformat()
            }

        return {
            "status": "available",
            "description": "Available to queue",
            "timestamp": datetime.datetime.utcnow().isoformat()
        }

    except Exception as e:
        print(f"Error getting player queue status: {e}")
        return {
            "status": "error",
            "error": str(e),
            "timestamp": datetime.datetime.utcnow().isoformat()
...
def get_channel_queue_status(channel_id):
    ...
    try:
        ...
        return {
            "channel_id": channel_id,
            "total_players": len(queue_players),
            "players_needed": max(0, 6 - len(queue_players)),
            "players": [
                {
                    "id": p.get("id"),
                    "name": p.get("name"),
            ...
            "is_ready": len(queue_players) >= 6,
            "timestamp": datetime.datetime.utcnow().isoformat()
        }
    except Exception as e:
        print(f"Error getting channel queue status: {e}")
        return {"error": str(e)}


# Broadcast functions for queue events
def broadcast_player_status_update(player_id):
    """Broadcast status update to a specific player"""
    try:
        player_id = str(player_id)
        status = get_player_queue_status(player_id)
        socketio.emit('queue_status_update', status, room=f'user_{player_id}')
        print(f"Broadcasted status update to user {player_id}: {status['status']}")
    except Exception as e:
        print(f"Error broadcasting player status: {e}")


def broadcast_queue_update(channel_id):
    """Broadcast queue update to all clients watching a channel"""
    try:
        channel_id = str(channel_id)
        queue_status = get_channel_queue_status(channel_id)
        socketio.emit('queue_update', queue_status, room=f'queue_{channel_id}')
        print(f"Broadcasted queue update for channel {channel_id}")

        # Also update individual players in this queue
        queue_players = queue_collection.find({"channel_id": channel_id})
...
def broadcast_match_update(match_id):
    """Broadcast match status update to all players in the match"""
    try:
        ...
        for team in ['team1', 'team2']:
            if team in match:
                all_players.extend([p.get('id') for p in match[team] if p.get('id')])

        # Also check the general players list
        if 'players' in match:
            all_players.extend([p.get('id') for p in match['players'] if p.get('id')])

        # Broadcast to each player
        for player_id in set(all_players):  # Remove duplicates
            if player_id:
                broadcast_player_status_update(player_id)

        print(f"Broadcasted match update for match {match_id} to {len(set(all_players))} players")
...
def get_match_phase_display(status):
    """Convert match status to user-friendly phase"""
    phase_map = {
        "voting": "Team Selection",
        "selection": "Captain Picks",
        "in_progress": "Playing Match",
        "completed": "Match Complete"
    }
    return phase_map.get(status, "In Match")


def get_match_description(status):
    """Get detailed description of match phase"""
    descriptions = {
        "voting": "Voting on team selection method",
        "selection": "Captains are picking teams",
        "in_progress": "Match in progress",
        "completed": "Match completed"
    }
    return descriptions.get(status, "Active match")


def estimate_queue_wait_time(queue_size):
    """Estimate wait time based on queue size"""
    if queue_size < 6:
        return f"Waiting for {6 - queue_size} more players"
    elif queue_size == 6:
        return "Match starting soon!"
    else:
        matches_ahead = (queue_size - 6) // 6
        if matches_ahead <= 1:
            return "Next match"
        else:
            estimated_minutes = matches_ahead * 15
            return f"~{estimated_minutes} minutes"


# REST API endpoint for initial status (fallback)
@app.route('/api/queue-status/<player_id>')
def get_queue_status_api(player_id):
    """REST API endpoint for queue status (fallback)"""
    status = get_player_queue_status(player_id)
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='0.0.0.0', port=port, debug=is_local)
